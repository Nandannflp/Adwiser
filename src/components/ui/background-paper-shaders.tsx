...
export function ShaderPlane({
  ...
  useFrame((state) => {
    if (mesh.current) {
      // SLOW SPEED (time / 3)
      uniforms.time.value = state.clock.elapsedTime / 3
...
export function EnergyRing({
  ...
  useFrame((state) => {
    if (mesh.current) {
      mesh.current.rotation.z = state.clock.elapsedTime / 3
      ;(mesh.current.material as any).opacity = 0.3 + Math.sin(state.clock.elapsedTime) * 0.2
    }
  })
